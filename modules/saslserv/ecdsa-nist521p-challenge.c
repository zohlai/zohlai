 ...
 * Copyright (c) 2015 Max Teufel <mteufel@urandom.eu.org>
 * Rights to this code are as documented in doc/LICENSE.
 *
 * ECDSA-NIST521P-CHALLENGE mechanism provider.
 * XXX Please do not use this in production unless nickserv/set_pubkey supports P-521 keys.
 */
...
sasl_mechanism_t mech = {"ECDSA-NIST521P-CHALLENGE", &mech_start, &mech_step, &mech_finish};
...
static int mech_step_accname(sasl_session_t *p, char *message, size_t len, char **out, size_t *out_len)
{
	...
	/* XXX Add an interface for this (nickserv/set_pubkey) */
	md = metadata_find(mu, "ecdsa-nist521p-pubkey");
	if (md == NULL)
	{
		return ASASL_FAIL;
	}

 ...
 * Copyright (c) 2014 Max Teufel <max@teufelsnetz.com>
 *
...
DECLARE_MODULE_V1
(
	"exttarget/ssl", false, _modinit, _moddeinit,
	PACKAGE_STRING,
...
static myentity_t *ssl_validate_f(const char *param)
{
	return &dummy_entity;
}
...
void _modinit(module_t *m)
{
	...
	mowgli_patricia_add(*exttarget_tree, "ssl", ssl_validate_f);

	object_init(object(&dummy_entity), "$ssl", NULL);
}

void _moddeinit(module_unload_intent_t intent)
{
	mowgli_patricia_delete(*exttarget_tree, "ssl");
}
